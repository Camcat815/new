...
class Listeo_Core_Bookings_Calendar {
    ...
    */
        ...
        if ($hour_end && $hour_start &&  get_post_meta($_POST['listing_id'], '_count_by_hour',true)) {
            ...
            if (get_post_meta($_POST['listing_id'], '_rental_timepicker', true)) {
                ...
                if(get_post_meta($_POST['listing_id'], '_count_by_hour', true)){
                    ...
                    $price = self::calculate_price_by_hours($_POST['listing_id'],  $_POST['date_start'], $_POST['date_end'], $hours, $multiply, $services, '');    
                } else {
    ...
    public  function ajax_validate_coupon(){
        ...
        // if price is in format like 2.380 make sure it gets to the function as 2380
...
//        if user dont have listings show his reservations
    ...
    */
    ...
    public static function calculate_price_by_hours($listing_id, $date_start, $date_end, $hours, $multiply = 1, $services = false, $coupon = false)
    {
        ...
        if ($_count_per_guest) {
            $price = $price * max(1, (int) $multiply);
        }
                ...
                case 'rental' :
                    ...
                    if ( $free_places > 0 ) 
                    {
                        ...
                        if ($count_by_hour) {
                            ...
                            $price = self::calculate_price_by_hours($data['listing_id'],  $data['date_start'], $data['date_end'], $hours, $multiply, $services, $coupon);
                            $price_before_coupons = self::calculate_price_by_hours($data['listing_id'],  $data['date_start'], $data['date_end'], $hours, $multiply, $services, '');
                        } else {
    ...
    public function booking_confirmation_form($values, $registration_errors = null) {
        ...
        if (get_post_meta($data['listing_id'], '_count_by_hour', true)) {
            ...
            if (get_post_meta($data['listing_id'], '_rental_timepicker', true)) {
                ...
                $data['price'] = self::calculate_price_by_hours($data['listing_id'],  $data['date_start'], $data['date_end'], $hours, $multiply, $services, '');
                
                if (!empty($coupon)) {
                    $data['price_sale'] = self::calculate_price_per_hour($data['listing_id'],  $data['date_start'], $data['date_end'], $hours, $multiply, $services, $coupon);
                }
            } else {
