	...
	?>
						...
						?>
							<li class="highlighted" id="date">
								...
								<?php 
								// check if current data is passed the $data->date_start or $data->date_end if it exists
								// if so, show the date as expired
								$today = date('Y-m-d H:i:s');
								if ($today > $data->date_start || $today > $data->date_end) {
									$show_renew = false;
								}
...
<?php endif; ?>
	...
	<div class="buttons-to-right booking-buttons-actions">
		<?php
		if($data->status != 'paid') {
		if ( $payment_method == 'cod' || $payment_method == 'cheque')  { ?>
